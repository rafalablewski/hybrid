 ...
 * it that way. A reviewer cannot hold sixteen values in their head across the
 * set, so three things are mechanical instead:
 *
 *   1. every card in the set renders its footer through the primitive;
 *   2. no footer takes the accent — with one link kind and nothing that
 *      navigates, there is nothing left for lime to mean;
 ...
 * ships only the admin panel, so the mobile cards are the whole set.)
 */
...
/** The cards whose footers this spec covers. */
const CARDS = [
...
describe("card footers go through the CardFoot primitive", () => {
  ...
  /**
   * A hand-rolled rule at the bottom of a card is how all of these started,
   * so the guard has to be able to SEE one. A flat regex cannot: the old
   * footers wrote the same three declarations in different orders, some
   * put `border: 0,` between the padding and the border, and interpolations
   * like `${C("line")}` have a closing brace that terminates any `[^}]*`
   * span. A first cut of this test used exactly that regex and caught none;
   * it passed because there was nothing left to find, which is the
   ...
   * padding, in any order.
   *
   * Tissue's protocol row is the one allowed match: it is a status row in the
   * card BODY, and it points at Today because a daily protocol — steps, dates,
   * checkboxes — cannot unfold into an analytics card.
   */
  ...
  for (const [name, path] of CARDS) {
    it(`${name} hand-rolls no footer rule beyond the protocol row`, () => {
      ...
      // Only the Tissue card may match, and only once.
  ...
  /**
   * And the guard must be capable of failing. Every old mobile footer is
   * reconstructed here in the shape it actually shipped in — if a future
   * refactor of `styleObjects`/`isFooterRule` stops seeing these, the test
   * above silently becomes decorative again.
   */
  it("the scanner sees each of the footers it replaced", () => {
    const SHIPPED = [
...
describe("nothing in a footer takes the accent", () => {
  /**
   * Read from the primitive itself rather than the callers: a caller
  ...
  it("the primitive exposes no colour, glyph or kind knob", () => {
    const kit = src(mob("kit.tsx"));
    const s = kit.slice(kit.indexOf("function FootChevron("));
...
describe("the retired vocabulary stays retired", () => {
  ...
  it("the Level card no longer navigates out of itself", () => {
    expect(src(mob("level-card.tsx"))).not.toContain("onOpenWorking");
...
describe("the injury sheet stays reachable", () => {
   ...
   */
  it("tissue-card offers the body and the pill with no data", () => {
    const s = src(mob("tissue-card.tsx"));
