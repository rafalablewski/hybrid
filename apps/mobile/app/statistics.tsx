    ...
    <AuroraScreen>
      <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start" }}>
        <View style={{ flexDirection: "row", gap: 10, alignItems: "center" }}>
          ...
          <AHeading style={{ fontSize: 26 }}>Your{"\n"}Statistics</AHeading>
        </View>
        <View style={{ alignItems: "flex-end", marginTop: 6 }}>
          <Text style={{ fontFamily: F.mono, fontSize: 10, color: C.ash }}>Weekly volume</Text>
          <View style={{ flexDirection: "row", alignItems: "center", gap: 6, marginTop: 2 }}>
            <AuroraIcon name="arrow-up" size={16} color={txt(C, C.lime)} />
            <Text style={{ fontFamily: F.black, fontSize: 18, color: C.chalk }}>{Math.round(recap.volume)} kg</Text>
          </View>
        </View>
      </View>
      ...
      <ACard style={{ marginTop: 18, paddingBottom: 26 }}>
        <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
          <Text style={{ fontFamily: F.bold, fontSize: 16, color: C.chalk }}>Sessions</Text>
          <Text style={{ fontFamily: F.mono, fontSize: 11, color: C.ash }}>{buckets.total} in {range}</Text>
        </View>
      ...
      {!hasData && (
        <Text style={{ fontFamily: F.reg, fontSize: 13, color: C.ash, textAlign: "center", marginTop: 18, lineHeight: 19 }}>
...
function Mini({ icon, label, value, color }: { icon: Parameters<typeof AuroraIcon>[0]["name"]; label: string; value: string; color: string }) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.ink2, borderWidth: 1, borderColor: C.line, borderRadius: RADIUS.card, padding: 16 }}>
      ...
      <Text style={{ fontFamily: F.mono, fontSize: 10, color: C.ash, marginTop: 8 }}>{label}</Text>
      <Text style={{ fontFamily: F.black, fontSize: 20, color: C.chalk, marginTop: 2 }}>{value}</Text>
    </View>
  );
}
