    ...
    <div>
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 14, flexWrap: "wrap" }}>
        ...
        <a
          ...
          style={{ ...mono, fontSize: 13, padding: "7px 12px", borderRadius: 999, cursor: "pointer", border: `1px solid ${LINE}`, background: "transparent", color: txt(ASH), marginLeft: "auto", textDecoration: "none" }}
        >
          ⬇ CSV
        </a>
        <button onClick={() => window.print()} style={{ ...mono, fontSize: 13, padding: "7px 12px", borderRadius: 999, cursor: "pointer", border: `1px solid ${LINE}`, background: "transparent", color: txt(ASH) }}>
          🖨 PDF
        </button>
        <button onClick={load} style={{ ...mono, fontSize: 13, padding: "7px 12px", borderRadius: 999, cursor: "pointer", border: `1px solid ${LINE}`, background: "transparent", color: txt(ASH) }}>
          ↻ refresh
        </button>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {runs?.map((r) => (
          <Card key={r.id} style={{ padding: 0, overflow: "hidden" }}>
            <details>
              <summary style={{ ...mono, fontSize: 13, color: CHALK, cursor: "pointer", listStyle: "none", padding: "12px 16px" }}>
              ...
              <div style={{ padding: "0 16px 14px", borderTop: `1px solid ${LINE}` }}>
                <Mono s={{ fontSize: 11, textTransform: "uppercase", letterSpacing: ".1em", display: "block", margin: "12px 0 4px" }} c={ASH}>
                  Task
                </Mono>
                <div style={{ ...mono, fontSize: 13, color: CHALK, whiteSpace: "pre-wrap", marginBottom: 10 }}>{r.task}</div>
                {r.steps.map((s, i) => (
                  <div key={i} style={{ marginBottom: 8, paddingLeft: 8, borderLeft: `2px solid ${VIOLET}` }}>
                    <Mono s={{ fontSize: 11, textTransform: "uppercase", display: "block" }} c={VIOLET}>↳ {s.role} — {s.agent}</Mono>
                    <div style={{ ...mono, fontSize: 12, color: CHALK, whiteSpace: "pre-wrap" }}>{s.output}</div>
                  </div>
                ))}
                <Mono s={{ fontSize: 11, textTransform: "uppercase", letterSpacing: ".1em", display: "block", margin: "4px 0" }} c={ASH}>
                  Output
                </Mono>
                <div style={{ ...mono, fontSize: 14, lineHeight: 1.6, color: CHALK, background: INK, border: `1px solid ${LINE}`, borderRadius: "var(--r-card)", padding: 14, whiteSpace: "pre-wrap" }}>
                  {r.output || "(no output)"}
                </div>
