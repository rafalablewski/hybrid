    ...
    <>
      ...
      {e1rmData.length > 1 ? (
        <div style={card}>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={e1rmData}><CartesianGrid stroke={LINE_HEX} strokeDasharray="3 3" /><XAxis dataKey="w" stroke={ASH} style={{ ...mono, fontSize: fs.micro }} /><YAxis stroke={ASH} style={{ ...mono, fontSize: fs.micro }} domain={["auto", "auto"]} width={44} /><Tooltip contentStyle={tip} formatter={(v) => `${v} ${units}`} /><Line type="monotone" dataKey="e1rm" stroke={LIME_HEX} strokeWidth={2.5} dot={{ r: 3 }} /></LineChart>
          </ResponsiveContainer>
        </div>
