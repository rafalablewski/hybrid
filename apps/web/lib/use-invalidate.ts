...
import { foodLogsKey } from "./use-food-logs";
...
export function useRevalidate() {
  ...
  return {
     ...
     *  that drive the Performance State on Today. A nutrition log also changed
     *  the diary ENTRIES that Today's Fuel plate reads, so that key drops too. */
    recovery: () =>
      Promise.all([
        qc.invalidateQueries({ queryKey: signalsKey }),
        qc.invalidateQueries({ queryKey: biometricsKey }),
        qc.invalidateQueries({ queryKey: foodLogsKey }),
      ]),
