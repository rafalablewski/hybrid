...
export default function AuroraCoach() {
  ...
  const sectionLabel = (text: string, color: string) => (
    <Text style={{ fontFamily: F.mono, fontSize: 11, textTransform: "uppercase", letterSpacing: 1.2, color, marginTop: 20, marginBottom: 10 }}>
      {text}
    </Text>
  );
  ...
  return (
    <AuroraScreen refreshing={refreshing} onRefresh={() => load(true)}>
      <AHeading style={{ fontSize: 26 }}>{t("nav.coach")}</AHeading>

      {loading ? (
        <Text style={{ fontFamily: F.mono, fontSize: 12, color: C.ash, marginTop: 18 }}>…</Text>
      ) : (
        <>
          {incoming.length > 0 && (
            <>
              ...
              {incoming.map((l) => (
                <ACard key={l.id} style={{ marginBottom: 12 }}>
                  <Text style={{ fontFamily: F.bold, fontSize: 16, color: C.chalk }}>{personName(l.coach)}</Text>
                  <Text style={{ fontFamily: F.mono, fontSize: 12, color: C.ash, marginTop: 4, marginBottom: 12 }}>{t("coach.wantsToCoach")}</Text>
                  <View style={{ flexDirection: "row", gap: 10, alignItems: "center" }}>
                    ...
                    <Pressable onPress={() => act(l.id, "end")} style={{ paddingHorizontal: 14, justifyContent: "center" }}>
                      <Text style={{ fontFamily: F.mono, fontSize: 12, color: C.ash }}>{t("common.decline")}</Text>
                    </Pressable>
                  </View>
                </ACard>
              ))}
          ...
          {coaches.length === 0 ? (
            <Text style={{ fontFamily: F.mono, fontSize: 12, color: C.ash }}>{t("coach.noCoach")}</Text>
          ) : (
            coaches.map((l) => (
              <ACard key={l.id} style={{ marginBottom: 12 }}>
                <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <Pressable onPress={() => act(l.id, "end")}>
                    <Text style={{ fontFamily: F.mono, fontSize: 12, color: C.ash }}>{t("common.end")}</Text>
                  </Pressable>
                </View>
              </ACard>
            ))
          ...
          <ACard>
            <TextInput
              ...
              style={{ fontFamily: F.mono, fontSize: 14, color: C.chalk, backgroundColor: C.ink, borderWidth: 1, borderColor: C.line, borderRadius: RADIUS.field, padding: 14, marginBottom: 12 }}
            />
            <APill label={t("common.invite")} variant="primary" onPress={invite} style={{ paddingVertical: 14 }} />
            {msg && (
              <Text style={{ fontFamily: F.mono, fontSize: 12, color: txt(C, msg.ok ? C.lime : C.amber), marginTop: 10 }}>
                {msg.text}
              </Text>
            )}
          </ACard>
          ...
          {clients.map((l) => (
            <Pressable key={l.id} onPress={() => setOpen(l)} style={{ marginTop: 12 }}>
              <ACard>
                <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
                  <View>
                    <Text style={{ fontFamily: F.bold, fontSize: 16, color: C.chalk }}>{personName(l.client)}</Text>
                    <Text style={{ fontFamily: F.mono, fontSize: 12, color: C.ash, marginTop: 2 }}>{l.client?.email}</Text>
                  </View>
                  <Text style={{ fontFamily: F.mono, fontSize: 12, color: txt(C, C.lime) }}>{t("common.open")} →</Text>
                </View>
              </ACard>
            </Pressable>
          ))}
            ...
            <ACard key={l.id} style={{ marginTop: 12 }}>
              <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <View style={{ backgroundColor: `${C.amber}1f`, borderRadius: RADIUS.pill, paddingHorizontal: 12, paddingVertical: 5 }}>
                  <Text style={{ fontFamily: F.mono, fontSize: 11, color: txt(C, C.amber) }}>{t("coach.pending")}</Text>
                </View>
              </View>
            </ACard>
          ))}
  ...
  return (
    <AuroraScreen>
      <Pressable onPress={back} style={{ marginBottom: 10 }}>
        <Text style={{ fontFamily: F.mono, fontSize: 12, color: C.ash }}>← {t("coach.roster")}</Text>
      </Pressable>
      ...
      <ACard>
        <TextInput
          ...
          placeholderTextColor={C.ash}
          multiline
          style={{ fontFamily: F.mono, fontSize: 14, color: C.chalk, backgroundColor: C.ink, borderWidth: 1, borderColor: C.line, borderRadius: RADIUS.field, padding: 14, minHeight: 56 }}
        />
        <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginTop: 12 }}>
          <Pressable onPress={() => setIsPrivate((p) => !p)} style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
            ...
            <Text style={{ fontFamily: F.mono, fontSize: 12, color: txt(C, isPrivate ? C.amber : C.ash) }}>{t("coach.private")}</Text>
          </Pressable>
      ...
      {loading ? (
        <Text style={{ fontFamily: F.mono, fontSize: 12, color: C.ash, marginTop: 16 }}>…</Text>
      ) : (
        <>
          {notes.map((n) => (
            <ACard key={n.id} style={{ marginTop: 12, borderLeftWidth: 3, borderLeftColor: n.private ? C.amber : C.line }}>
              ...
              <Text style={{ fontFamily: F.mono, fontSize: 14, color: C.chalk, marginTop: n.private ? 8 : 0, lineHeight: 20 }}>{n.body}</Text>
            </ACard>
          ))}

          {sessions.length > 0 && <ClientWeek sessions={sessions} t={t} />}

          <Text style={{ fontFamily: F.mono, fontSize: 11, textTransform: "uppercase", letterSpacing: 1.2, color: txt(C, C.lime), marginTop: 20, marginBottom: 10 }}>{t("coach.recentSessions")}</Text>
          {sessions.length === 0 ? (
            <Text style={{ fontFamily: F.mono, fontSize: 12, color: C.ash }}>{t("coach.noSessions")}</Text>
          ) : (
            sessions.map((s) => (
              <ACard key={s.id} style={{ marginBottom: 12 }}>
                <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                  <Text style={{ fontFamily: F.semi, fontSize: 15, color: C.chalk }}>{s.title}</Text>
                  <Text style={{ fontFamily: F.mono, fontSize: 12, color: C.ash }}>{new Date(s.startedAt).toLocaleDateString()}</Text>
                </View>
                <View style={{ flexDirection: "row", gap: 8, marginTop: 8 }}>
                  <View style={{ backgroundColor: C.ink, borderRadius: RADIUS.pill, paddingHorizontal: 11, paddingVertical: 4, borderWidth: 1, borderColor: C.line }}>
                    <Text style={{ fontFamily: F.mono, fontSize: 11, color: C.ash }}>{sessionVolume(s.blocks).toLocaleString()} kg</Text>
                  </View>
                  <View style={{ backgroundColor: C.ink, borderRadius: RADIUS.pill, paddingHorizontal: 11, paddingVertical: 4, borderWidth: 1, borderColor: C.line }}>
                    <Text style={{ fontFamily: F.mono, fontSize: 11, color: C.ash }}>{s.blocks.length} blocks</Text>
                  </View>
                </View>
              </ACard>
            ))
...
function ClientWeek({ sessions, t }: { sessions: LoggedSession[]; t: (k: string) => string }) {
  ...
  return (
    <>
      <Text style={{ fontFamily: F.mono, fontSize: 11, textTransform: "uppercase", letterSpacing: 1.2, color: txt(C, C.lime), marginTop: 20, marginBottom: 10 }}>{t("recap.title")}</Text>
      <ACard>
        {r.sessions === 0 ? (
          <Text style={{ fontFamily: F.mono, fontSize: 12, color: C.ash }}>{t("recap.noneThisWeek")}</Text>
        ) : (
          <>
            ...
            {hasPrev && (
              <Text style={{ fontFamily: F.mono, fontSize: 12, color: txt(C, r.volumeDelta >= 0 ? C.lime : C.amber), marginTop: 14 }}>
                {r.sessionsDelta >= 0 ? "+" : ""}{r.sessionsDelta} {t("recap.sessions")} · {r.volumeDelta >= 0 ? "+" : ""}
                {r.volumeDelta.toLocaleString()} kg {t("recap.vsLastWeek")}
              </Text>
            )}
            {r.prs.length > 0 && (
              <Text style={{ fontFamily: F.mono, fontSize: 12, color: C.chalk, marginTop: 10, lineHeight: 18 }}>
