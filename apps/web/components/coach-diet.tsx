...
import { LIME, ASH, CHALK, LINE, INK2, mono, Mono, Card } from "@/lib/ui";
...
export default function CoachDiet({ linkId }: { linkId: string }) {
  ...
  const field = (label: string, key: keyof Diet, unit: string) => (
    <div style={{ flex: "1 1 90px" }}>
      <Mono s={{ fontSize: 10, textTransform: "uppercase", letterSpacing: ".06em", display: "block", marginBottom: 4 }} c={ASH}>{label}</Mono>
      <input
        value={d[key]}
        onChange={(e) => setD((p) => ({ ...p, [key]: e.target.value.replace(/[^0-9]/g, "") }))}
        inputMode="numeric"
        placeholder={unit}
        style={{ ...mono, fontSize: 14, width: "100%", padding: "9px 10px", borderRadius: 10, background: INK2, color: CHALK, border: `1px solid ${LINE}`, outline: "none" }}
      />
    </div>
  );
  ...
  return (
    <Card style={{ borderLeft: `3px solid ${LIME}` }}>
      <Mono s={{ fontSize: 11, textTransform: "uppercase", letterSpacing: ".1em" }} c={LIME}>Assign diet · daily macros</Mono>
      ...
      <input
        ...
        style={{ ...mono, fontSize: 13, width: "100%", marginTop: 8, padding: "9px 10px", borderRadius: 10, background: INK2, color: CHALK, border: `1px solid ${LINE}`, outline: "none" }}
      />
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 10, gap: 10, flexWrap: "wrap" }}>
        <button onClick={save} style={{ ...mono, fontSize: 13, fontWeight: 700, color: "#0c0d0c", background: LIME, border: "none", borderRadius: 10, padding: "9px 18px", cursor: "pointer" }}>
          Save diet
        </button>
        {msg && <Mono s={{ fontSize: 12 }} c={LIME}>{msg}</Mono>}
      </div>
      {unavailable && (
