...
export default function AuroraTactical() {
  ...
  return (
    <AuroraScreen>
      <AHeading style={{ fontSize: 26 }}>Deployment readiness</AHeading>
      <Text style={{ fontFamily: F.reg, fontSize: 14, color: C.ash, marginTop: 8, lineHeight: 20 }}>
        Fuses the Twin (HPI + injury availability) with occupational capacity into a duty status, plus a unit go/no-go.
      </Text>

      <ACard style={{ marginTop: 16 }}>
        <Text style={{ fontFamily: F.mono, fontSize: 11, textTransform: "uppercase", letterSpacing: 1.2, color: txt(C, C.blue) }}>Your readiness</Text>
        {noData ? (
          <Text style={{ fontFamily: F.mono, fontSize: 13, color: C.chalk, marginTop: 8, lineHeight: 18 }}>Log training to compute your DRI from real readiness + injury risk.</Text>
        ) : dr ? (
          <>
            <View style={{ flexDirection: "row", alignItems: "baseline", gap: 10, marginTop: 6 }}>
              ...
              <View style={{ backgroundColor: `${statusColor(dr.status, C)}1f`, borderRadius: RADIUS.pill, paddingHorizontal: 11, paddingVertical: 4 }}>
                <Text style={{ fontFamily: F.mono, fontSize: 11, color: txt(C, statusColor(dr.status, C)) }}>{dr.status}</Text>
              </View>
            </View>
            <View style={{ flexDirection: "row", gap: 14, marginTop: 8 }}>
              <Text style={{ fontFamily: F.mono, fontSize: 12, color: C.ash }}>HPI {state!.hpi}</Text>
              <Text style={{ fontFamily: F.mono, fontSize: 12, color: C.ash }}>injury risk {state!.injuryRisk}</Text>
            </View>
            {dr.limiters.length > 0 && <Text style={{ fontFamily: F.mono, fontSize: 12, color: txt(C, C.amber), marginTop: 8, lineHeight: 18 }}>{dr.limiters.join(" · ")}</Text>}
          ...
          <Text style={{ fontFamily: F.mono, fontSize: 13, color: C.ash, marginTop: 8 }}>Loading…</Text>
        )}
      </ACard>
      ...
      {unit && (
        <ACard style={{ marginTop: 14 }}>
          <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
            <Text style={{ fontFamily: F.mono, fontSize: 11, textTransform: "uppercase", letterSpacing: 1.2, color: txt(C, unit.go ? C.lime : C.red) }}>Unit readiness</Text>
            <View style={{ backgroundColor: `${unit.go ? C.lime : C.red}1f`, borderRadius: RADIUS.pill, paddingHorizontal: 11, paddingVertical: 4 }}>
              <Text style={{ fontFamily: F.mono, fontSize: 11, color: txt(C, unit.go ? C.lime : C.red) }}>{unit.go ? "MISSION GO" : "NO-GO"} · {unit.pctReady}%</Text>
            </View>
          </View>
          <View style={{ marginTop: 10 }}>
            {unit.members.map((m) => (
              <View key={m.name} style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingVertical: 9, borderTopWidth: 1, borderTopColor: C.line }}>
                ...
                <Text style={{ fontFamily: F.mono, fontSize: 13, color: C.chalk, width: 44, textAlign: "right" }}>{m.dri}</Text>
                <View style={{ width: 120, alignItems: "flex-end" }}>
                  <View style={{ backgroundColor: `${statusColor(m.status, C)}1f`, borderRadius: RADIUS.pill, paddingHorizontal: 11, paddingVertical: 4 }}>
                    <Text style={{ fontFamily: F.mono, fontSize: 11, color: txt(C, statusColor(m.status, C)) }}>{m.status}</Text>
                  </View>
                </View>
              </View>
            ))}
          </View>
          <Text style={{ fontFamily: F.mono, fontSize: 11, color: C.ash, marginTop: 10 }}>Your readiness alongside an illustrative unit (real units plug in via the Org graph).</Text>
        </ACard>
      )}
