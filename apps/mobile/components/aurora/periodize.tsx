...
export default function AuroraPeriodize() {
  ...
  if (!macro) {
    return (
      <AuroraScreen refreshing={refreshing} onRefresh={load}>
        <AHeading style={{ fontSize: 26 }}>No active plan</AHeading>
  ...
  return (
    <AuroraScreen refreshing={refreshing} onRefresh={load}>
      <Text style={{ fontFamily: F.mono, fontSize: 11, textTransform: "uppercase", letterSpacing: 1.2, color: txt(C, C.lime) }}>{macro.goalOrSport}{macro.model ? ` · ${macro.model}` : " · enrolled"}</Text>
      <AHeading style={{ fontSize: 26, marginTop: 6 }}>{macro.totalWeeks}-week season</AHeading>
      <Text style={{ fontFamily: F.mono, fontSize: 13, color: C.ash, marginTop: 6 }}>Now in {current.label} · week {week}/{macro.totalWeeks}</Text>
        ...
        <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 12, marginTop: 12 }}>
          {macro.blocks.map((b) => (
            <View key={b.key} style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
              ...
              <Text style={{ fontFamily: F.mono, fontSize: 11, color: b.key === current.key ? C.chalk : C.ash }}>{b.label}</Text>
            </View>
          ))}
        </View>
      </ACard>
      ...
      {macro.blocks.map((b) => (
        <ACard key={b.key} style={{ marginTop: 14 }}>
          <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "baseline" }}>
            ...
            <Text style={{ fontFamily: F.mono, fontSize: 12, color: C.ash }}>wk {b.startWeek}–{b.endWeek}</Text>
          </View>
          <Text style={{ fontFamily: F.mono, fontSize: 12, color: C.chalk, marginTop: 6, marginBottom: 12, lineHeight: 18 }}>{b.focus}</Text>
          <View style={{ flexDirection: "row", gap: 6 }}>
            {b.micros.map((m) => (
              ...
              >
                <Text style={{ fontFamily: F.mono, fontSize: 10, color: m.kind === "recovery" ? C.ash : C.chalk }}>W{m.week}</Text>
