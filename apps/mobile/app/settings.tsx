...
function ClassicSettings() {
  ...
  const C = palette;
  const inputStyle = { fontFamily: F.mono, fontSize: fs.note, color: C.chalk, backgroundColor: C.ink2, borderWidth: 1, borderColor: C.line, borderRadius: 10, paddingHorizontal: 12, paddingVertical: 11 } as const;
  ...
  return (
    <Screen>
      ...
      <Text style={{ fontFamily: F.black, fontSize: fs.display, color: C.chalk, marginTop: 10 }}>{t("settings.title")}</Text>

      {/* Account identity */}
      <Card style={{ marginTop: 16 }}>
        <Kicker color={C.blue}>Account</Kicker>
        <Text style={{ fontFamily: F.bold, fontSize: 17, color: C.chalk, marginTop: 8 }}>{name}</Text>
        <View style={{ flexDirection: "row", gap: space.sm, marginTop: 10, flexWrap: "wrap" }}>
          <View style={{ borderWidth: 1, borderColor: `${C.violet}55`, backgroundColor: `${C.violet}1a`, borderRadius: 6, paddingHorizontal: 8, paddingVertical: 2 }}>
            <Text style={{ fontFamily: F.mono, fontSize: fs.nano, color: C.violet, letterSpacing: 0.5 }}>{role.toUpperCase()}</Text>
          </View>
          <View style={{ borderWidth: 1, borderColor: entitlement === "paid" ? C.lime : C.line, backgroundColor: entitlement === "paid" ? `${C.lime}1a` : "transparent", borderRadius: 6, paddingHorizontal: 8, paddingVertical: 2 }}>
            <Text style={{ fontFamily: F.mono, fontSize: fs.nano, color: entitlement === "paid" ? C.lime : C.ash, letterSpacing: 0.5 }}>{entitlement === "paid" ? "FULL · PAID" : "FREE"}</Text>
          </View>
        </View>
        {!!acct.email && <Mono color={C.chalk} style={{ marginTop: 10, fontSize: fs.caption }}>{acct.email}</Mono>}
      </Card>

      {/* Edit profile — name / email, on the user's own Supabase auth */}
      <Card style={{ marginTop: 16 }}>
        <Kicker color={C.lime}>Edit profile</Kicker>
        <Mono color={C.ash} style={{ marginTop: 12, marginBottom: 6, fontSize: fs.micro }}>Display name</Mono>
        <View style={{ flexDirection: "row", gap: space.sm }}>
          ...
          <Pressable onPress={acct.saveName} disabled={acct.busy} style={btn(C.lime)}><Text style={{ fontFamily: F.mono, fontSize: fs.body, color: txt(C, C.lime) }}>Save</Text></Pressable>
        </View>
        <Mono color={C.ash} style={{ marginTop: 14, marginBottom: 6, fontSize: fs.micro }}>Change email</Mono>
        <View style={{ flexDirection: "row", gap: space.sm }}>
          ...
          <Pressable onPress={acct.changeEmail} disabled={acct.busy || !acct.newEmail.trim()} style={btn(C.ash)}><Text style={{ fontFamily: F.mono, fontSize: fs.body, color: C.ash }}>Update</Text></Pressable>
        </View>
        {!!acct.profileMsg && <Mono color={acct.profileMsg.startsWith("✓") ? C.lime : C.ash} style={{ marginTop: 10, fontSize: fs.caption }}>{acct.profileMsg}</Mono>}
        {!acct.authOn && <Mono color={C.ash} style={{ marginTop: 8, fontSize: fs.micro }}>Profile editing needs a real signed-in account.</Mono>}
      </Card>
        ...
        <View style={{ flexDirection: "row", gap: space.sm, marginTop: 12 }}>
          {APPEARANCE.map((opt) => {
            ...
            return (
              ...
              >
                <Text style={{ fontFamily: F.bold, fontSize: fs.body, color: on ? C.onAccent : C.ash }}>{opt.label}</Text>
              </Pressable>
            );
          })}
        </View>
      ...
      <Card style={{ marginTop: 16 }}>
        ...
        <Mono style={{ marginTop: 4, fontSize: fs.micro }}>Switch the whole app between the classic look and the new rounded design.</Mono>
        <View style={{ gap: space.sm, marginTop: 12 }}>
          {TEMPLATES.map((opt) => {
            ...
            return (
              ...
              >
                <Text style={{ fontFamily: F.bold, fontSize: fs.note, color: on ? txt(C, C.lime) : C.chalk }}>{opt.label}</Text>
                <Mono style={{ marginTop: 3, fontSize: fs.micro, lineHeight: 16 }}>{opt.description}</Mono>
              </Pressable>
            );
          })}
        </View>
      ...
      <Pressable onPress={() => router.push("/logger-settings")}>
        <Card style={{ marginTop: 16, flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
          <View style={{ flex: 1 }}>
            ...
            <Mono style={{ marginTop: 4, fontSize: fs.micro }}>{t("loggerPrefs.intro")}</Mono>
          </View>
          <Text style={{ fontFamily: F.black, fontSize: fs.title, color: C.ash, marginLeft: 10 }}>→</Text>
        </Card>
      </Pressable>
      ...
      <Card style={{ marginTop: 16 }}>
        ...
        <Mono color={C.ash} style={{ marginTop: 6, fontSize: fs.micro, lineHeight: 16 }}>What HYBRID may send you. Saved to your account &amp; synced across devices; honoured as each channel rolls out.</Mono>
        ...
        {!acct.authOn && <Mono color={C.ash} style={{ marginTop: 10, fontSize: fs.micro }}>Sign in with a real account to change these.</Mono>}
      </Card>

      {/* Privacy */}
      <Card style={{ marginTop: 16 }}>
        <Kicker color={C.lime}>Privacy</Kicker>
        <Mono color={C.ash} style={{ marginTop: 6, fontSize: fs.micro, lineHeight: 16 }}>You control what you share. Saved to your account &amp; synced across devices.</Mono>
        <View style={{ marginTop: 8 }}>
      ...
      <Card style={{ marginTop: 16 }}>
        ...
        {acct.provider && acct.provider !== "email" ? (
          <Mono color={C.chalk} style={{ marginTop: 10, fontSize: fs.caption, lineHeight: 17 }}>You sign in with {acct.provider} — manage your password there.</Mono>
        ) : (
          <>
            <Mono color={C.ash} style={{ marginTop: 12, marginBottom: 6, fontSize: fs.micro }}>Change password</Mono>
            <View style={{ flexDirection: "row", gap: space.sm }}>
              ...
              <Pressable onPress={acct.changePassword} disabled={acct.busy || acct.newPw.length < 8} style={btn(C.lime)}><Text style={{ fontFamily: F.mono, fontSize: fs.body, color: txt(C, C.lime) }}>Update</Text></Pressable>
            </View>
            {!!acct.passwordMsg && <Mono color={acct.passwordMsg.startsWith("✓") ? C.lime : C.ash} style={{ marginTop: 10, fontSize: fs.caption }}>{acct.passwordMsg}</Mono>}
          </>
        )}
        <Mono color={C.ash} style={{ marginTop: 16, fontSize: fs.micro, lineHeight: 16 }}>Sign out of every device — revokes all other sessions and ends this one.</Mono>
        <Pressable onPress={acct.signOutEverywhere} style={{ ...btn(C.ash), alignSelf: "flex-start", marginTop: 10, paddingVertical: 10 }}>
          <Text style={{ fontFamily: F.mono, fontSize: fs.body, color: C.ash }}>Sign out everywhere</Text>
        </Pressable>
      </Card>
      ...
      <Card style={{ marginTop: 16 }}>
        ...
        <Mono color={C.chalk} style={{ marginTop: 8, fontSize: fs.caption, lineHeight: 17 }}>Download everything tied to your account — sessions, signals, check-ins, plans, templates, events and more — as one JSON file.</Mono>
        <Pressable onPress={acct.exportData} disabled={acct.exportBusy} style={{ ...btn(C.lime), alignSelf: "flex-start", marginTop: 12, paddingVertical: 11 }}>
          {acct.exportBusy ? <ActivityIndicator color={txt(C, C.lime)} /> : <Text style={{ fontFamily: F.mono, fontSize: fs.body, color: txt(C, C.lime) }}>Download my data (JSON)</Text>}
        </Pressable>
        {!!acct.exportMsg && <Mono color={C.ash} style={{ marginTop: 10, fontSize: fs.caption }}>{acct.exportMsg}</Mono>}
      </Card>

      <Card style={{ borderLeftWidth: 3, borderLeftColor: C.red, marginTop: 16 }}>
        <Kicker color={C.red}>{t("settings.dangerZone")}</Kicker>
        <Text style={{ fontFamily: F.bold, fontSize: fs.title, color: C.chalk, marginTop: 8 }}>{t("settings.resetTitle")}</Text>
        ...
        <TextInput
          ...
          autoCorrect={false}
          style={{ fontFamily: F.mono, fontSize: fs.subtitle, color: C.chalk, backgroundColor: C.ink2, borderWidth: 1, borderColor: armed ? C.red : C.line, borderRadius: 10, paddingHorizontal: 12, paddingVertical: 11 }}
        />
        ...
        >
          {busy ? <ActivityIndicator color="#fff" /> : <Text style={{ fontFamily: F.black, fontSize: fs.note, color: "#fff" }}>{t("settings.eraseEverything")}</Text>}
        </Pressable>
      </Card>
