...
import { fs, space, LIME, CHALK, ASH, AMBER, RED, BLUE, VIOLET, disp, Mono, Card, Chip } from "@/lib/ui";
  ...
  return (
    <div style={{ display: "grid", gap: space.lg }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(min(100%, 260px), 1fr))", gap: 14 }}>
        {analyses.length === 0 && <Mono s={{ fontSize: fs.body }}>No analyses yet — record a clip on the phone to see the engine output.</Mono>}
        {analyses.map((a) => (
          <Card key={a.id}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
              <div style={{ ...disp, fontWeight: 800, fontSize: fs.title, textTransform: "capitalize", color: CHALK }}>{a.movement}</div>
              <div style={{ ...disp, fontWeight: 900, fontSize: 30, color: scoreColor(a.metrics.techniqueScore) }}>{a.metrics.techniqueScore}</div>
            </div>
            <Mono s={{ fontSize: fs.micro, display: "block", marginTop: 2 }} c={ASH}>technique score</Mono>
            ...
            {a.metrics.flags.length > 0 && (
              <div style={{ marginTop: 10 }}>
                {a.metrics.flags.map((f) => (
                  <Mono key={f} s={{ fontSize: fs.micro, display: "block" }} c={AMBER}>⚠ {f}</Mono>
                ))}
              </div>
            )}
            <Mono s={{ fontSize: fs.nano, display: "block", marginTop: 10 }} c={ASH}>{new Date(a.createdAt).toLocaleString()}</Mono>
          </Card>
        ))}
      </div>
    </div>
