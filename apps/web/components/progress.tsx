...
import { fs, space, INK2, LINE, LIME, CHALK, ASH, BLUE, RED, ON_ACCENT, disp, cond, mono, Mono, Card } from "@/lib/ui";
...
export default function Progress() {
  ...
  if (status === "no-bucket")
    return (
      <Card>
        <Mono s={{ fontSize: fs.body, lineHeight: 1.6, display: "block" }} c={RED}>
          The <strong>progress</strong> storage bucket isn&apos;t set up yet.
        </Mono>
        <Mono s={{ fontSize: fs.body, lineHeight: 1.6, display: "block", marginTop: 8 }}>
  ...
  return (
    <div style={{ maxWidth: 860 }}>
      <Card style={{ marginBottom: 16 }}>
        <Mono s={{ fontSize: fs.micro, textTransform: "uppercase", letterSpacing: ".1em" }} c={BLUE}>Progress photos</Mono>
        ...
        <button onClick={() => fileRef.current?.click()} disabled={busy}
          style={{ ...disp, fontWeight: 800, fontSize: fs.note, background: LIME, color: ON_ACCENT, border: "none", borderRadius: 12, padding: "12px 24px", cursor: busy ? "default" : "pointer", opacity: busy ? 0.5 : 1 }}>
          {busy ? "Uploading…" : "+ Add photo"}
        </button>
      </Card>

      {photos.length === 0 ? (
        <Mono s={{ fontSize: fs.body }}>No photos yet — add your first to start the timeline.</Mono>
      ) : (
          ...
          {photos.map((p) => (
            <div key={p.path} style={{ border: `1px solid ${LINE}`, borderRadius: 12, overflow: "hidden", background: INK2 }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 10px" }}>
                <Mono s={{ fontSize: fs.caption }}>{p.date}</Mono>
                <button onClick={() => remove(p.path)} style={{ ...cond, fontSize: fs.micro, color: ASH, background: "none", border: "none", cursor: "pointer" }}>delete</button>
              </div>
            </div>
          ))}
        </div>
