...
function GoalTileFace({ cover, pressed }: { cover: ReturnType<typeof goalCoverView>; pressed: boolean }) {
  return (
    ...
    >
      ...
      <Text numberOfLines={3} style={{ fontFamily: F.black, fontSize: 16, lineHeight: 18, letterSpacing: tracking.display, color: cover.ready ? "#fff" : "rgba(255,255,255,0.62)" }}>{cover.title}</Text>
    </View>
  );
}
                ...
                <View style={{ flexDirection: "row", gap: 16, marginTop: 12, marginBottom: 10 }}>
                  {hero.stats.map((s) => (
                    <View key={s.label} style={{ flex: 1, borderTopWidth: 2, borderTopColor: withAlpha(C.chalk, 0.14), paddingTop: 8 }}>
                      <Text style={{ fontFamily: F.black, fontSize: 20, lineHeight: 21, letterSpacing: tracking.display, color: C.chalk, fontVariant: ["tabular-nums"] }}>
                        {s.value}
