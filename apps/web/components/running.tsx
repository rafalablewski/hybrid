...
import { fs, space, INK2, LINE, LIME, CHALK, ASH, BLUE, AMBER, RED, disp, mono, tip, Mono, Card, ChartFrame, Select } from "@/lib/ui";
...
function Stat({ label, value, c = CHALK }: { label: string; value: string | number; c?: string }) {
  return (
    <Card>
      <div style={{ ...disp, fontWeight: 800, fontSize: fs.display, color: c }}>{value}</div>
      <Mono s={{ fontSize: fs.micro, textTransform: "uppercase", letterSpacing: ".08em", marginTop: 4 }}>{label}</Mono>
    </Card>
  );
}
...
export default function Running({ sessions }: { sessions: LoggedSession[] }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: space.lg, maxWidth: 980 }}>
      <Header />
      ...
      {hasEffort && (
        <Card>
          ...
          <Mono s={{ fontSize: fs.micro, lineHeight: 1.5, display: "block", marginTop: 10 }} c={ASH}>
      ...
      {paceMoves.length > 1 && (
        <Card>
          <Mono s={{ fontSize: fs.micro, textTransform: "uppercase", letterSpacing: ".08em", display: "block", marginBottom: 6 }}>Pace chart move</Mono>
      ...
      <Card>
        ...
        <div style={{ marginTop: 12, overflowX: "auto", maxWidth: "100%" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr 1fr", gap: space.sm, paddingBottom: 6, borderBottom: `1px solid ${LINE}` }}>
            {["move", "runs", "km", "longest", "best pace"].map((h) => (
              <Mono key={h} s={{ fontSize: fs.nano, textTransform: "uppercase" }}>{h}</Mono>
            ))}
          </div>
          {stats.map((r) => (
            <div key={r.move} style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr 1fr", gap: space.sm, padding: "8px 0", borderTop: `1px solid ${LINE}` }}>
              <Mono s={{ fontSize: fs.body }} c={CHALK}>{r.move}</Mono>
              <Mono s={{ fontSize: fs.body }}>{r.efforts}</Mono>
              <Mono s={{ fontSize: fs.body }}>{r.distanceKm}</Mono>
              <Mono s={{ fontSize: fs.body }}>{r.longestKm || "–"}</Mono>
              <Mono s={{ fontSize: fs.body }} c={r.bestPaceSecPerKm != null ? BLUE : ASH}>
...
function Header() {
  return (
    <div style={{ marginBottom: 4 }}>
      <div style={{ ...disp, fontWeight: 800, fontSize: fs.display }}>Running</div>
...
function Legend({ c, label }: { c: string; label: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: space.xs }}>
      ...
      <Mono s={{ fontSize: fs.caption }}>{label}</Mono>
    </div>
  );
}
