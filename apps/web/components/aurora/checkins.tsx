  ...
  return (
    <div style={{ maxWidth: "100%", margin: "0 auto", fontFamily: "var(--font-display)", color: C("chalk") }}>
      <div style={{ display: "flex", alignItems: "center", gap: space.ms }}>
        <h1 style={{ fontWeight: 900, fontSize: fs.display, margin: 0 }}>Daily check-in</h1>
        <span style={{ marginLeft: "auto" }}><AuroraIcon name="heart" size={24} color={C("red")} /></span>
      </div>

      <div style={{ ...card, marginTop: 18 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ fontFamily: "var(--font-mono)", fontSize: fs.micro, textTransform: "uppercase", letterSpacing: ".12em", color: C("lime") }}>This week</span>
        ...
        <div style={{ fontWeight: 900, fontSize: 28, marginTop: 6 }}>
          {compliance.completedThisWeek}<span style={{ color: C("ash"), fontSize: fs.title }}>/{compliance.target} sessions</span>
        </div>
        <div style={{ fontFamily: "var(--font-mono)", fontSize: fs.caption, color: C("ash") }}>{compliance.pct}% of plan · {compliance.compliantWeeks}-week streak</div>

        {RATINGS.map((r) => (
          <div key={r.key} style={{ marginTop: 16 }}>
            <div style={{ fontWeight: 700, fontSize: fs.body }}>{r.label}</div>
        ...
        <button onClick={() => isPaid && setForm((s) => ({ ...s, sharedWithCoach: !s.sharedWithCoach }))} disabled={!isPaid}
          ...
          <span style={{ flex: 1 }}>
            <span style={{ fontWeight: 700, fontSize: fs.body, display: "block" }}>Share with coach</span>
      ...
      <div style={{ fontFamily: "var(--font-mono)", fontSize: fs.micro, textTransform: "uppercase", letterSpacing: ".12em", color: C("blue"), margin: "18px 0 10px" }}>Check-in history</div>
      {history.length === 0 ? (
        <div style={{ fontSize: fs.body, color: C("ash") }}>No check-ins yet — submit today&apos;s above.</div>
      ) : (
        history.map((c) => (
          <div key={c.id} style={{ ...card, padding: 18, marginBottom: 11 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: space.sm }}>
                ...
                {c.adherencePct != null && <span style={{ fontFamily: "var(--font-mono)", fontSize: fs.micro, color: C("ash") }}>{c.adherencePct}% adherence</span>}
              </div>
            </div>
            ...
            {c.coachReply && (
              <div style={{ marginTop: 10, paddingLeft: 10 }}>
                <div style={{ fontFamily: "var(--font-mono)", fontSize: fs.micro, textTransform: "uppercase", letterSpacing: ".1em", color: C("violet") }}>Coach</div>
