...
export default function AuroraToday({
  ...
  // TIER-3 quick action, a slide-up sheet (not full-screen nav): the nutrition
  // tracker. (Readiness is now set inline on the feeling card, and Follow a
  // coach is a rail on the page rather than a row behind a sheet — see below.)
  ...
  return (
    <div style={{ maxWidth: "100%", margin: "0 auto", fontFamily: "var(--font-display)" }}>
      ...
      {/* ───── FUEL — the nutrition summary widget (one calendar-style stateful
          surface: empty → refuel / on-track / over → goal-hit, with a persistent
          quick-log rail). Shows on the real today only; a scrubbed past/future
          day scopes the cards above but nutrition targets are always today's.
          State + macros come from @hybrid/core fuelToday() so mobile matches.
          Tapping opens the same quick-add sheet. No section head — the widget
          titles itself. ───── */}
      {dayIsToday && <div style={{ marginTop: 12 }}><AuroraFuel sessions={sessions} onOpen={() => setNutritionOpen(true)} /></div>}

      {/* ───── FOLLOW A COACH — moved here from the Explore tab, which is gone.
          It is on the PAGE now rather than a row that opened a sheet: the rail
          sells coaches by showing them, and a row reading "Follow a coach" sold
          ...
          "See more" carrying through to the marketplace. Mirrors mobile. ───── */}
