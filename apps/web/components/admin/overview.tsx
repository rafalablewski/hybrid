...
export default function AdminOverview() {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16 }}>
      ...
      <Stat label="Organizations" value={s.orgs.toLocaleString()} c={BLUE} />
      ...
      <ChartFrame span={4} title="Growth" kicker="Last 12 weeks · signups vs sessions" c={LIME}>
        <ResponsiveContainer width="100%" height={240}>
          <LineChart data={s.growth}>
            ...
            <XAxis dataKey="week" stroke={ASH} style={{ ...mono, fontSize: 11 }} />
      ...
      <ChartFrame span={1} title="Roles" kicker="User base" c={AMBER}>
        <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 4 }}>
          {s.roleSplit.length === 0 && <Mono s={{ fontSize: 13 }}>No users yet.</Mono>}
          {s.roleSplit.map((r) => (
            <div key={r.role} style={{ display: "flex", justifyContent: "space-between" }}>
              <Mono s={{ fontSize: 13 }} c={roleColor[r.role] ?? CHALK}>{r.role}</Mono>
              <Mono s={{ fontSize: 12 }}>{r.n}</Mono>
            </div>
          ))}
        </div>
      </ChartFrame>
      ...
      <ChartFrame span={1} title="Languages" kicker="Users by locale" c={BLUE}>
        <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 4 }}>
          {s.langSplit.length === 0 && <Mono s={{ fontSize: 13 }}>—</Mono>}
          {s.langSplit.map((l) => (
            <div key={l.lang} style={{ display: "flex", justifyContent: "space-between" }}>
              <Mono s={{ fontSize: 13 }} c={CHALK}>{l.lang.toUpperCase()}</Mono>
