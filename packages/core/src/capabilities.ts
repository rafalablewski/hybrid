...
export const CAPABILITIES: Capability[] = [
  ...
  // Year 2 — Risk & Teams
  { id: "org-graph", area: "Backend", status: "shipped", title: "Team Operating System (Org Graph)", detail: "v1 shipped: Organization + self-referential Team hierarchy + Membership with roles (OWNER/DIRECTOR/COACH/MEDICAL/ANALYST/ATHLETE). Access scoped by role × data sensitivity in core (org.ts canRead/canManageOrg, unit-tested) and enforced in /api/org + /api/org/[id] (medical fields hidden from non-medical roles). Org screen renders the team tree + role-scoped roster; owners/directors manage teams. DB: prisma models + reference/sql-org-graph.sql (member-gated RLS). Next: invite staff/athletes, team-subtree scoping of athlete data, board reporting." },
