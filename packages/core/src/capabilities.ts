...
export const CAPABILITIES: Capability[] = [
  ...
  // Year 2 — Risk & Teams
  { id: "org-graph", area: "Backend", status: "shipped", title: "Team Operating System (Org Graph)", detail: "Organization + self-referential Team hierarchy + Membership with roles (OWNER/DIRECTOR/COACH/MEDICAL/ANALYST/ATHLETE). Access scoped by role × data sensitivity in core (org.ts, unit-tested). Operational: managers invite staff/athletes by email, assign roles + teams (last-owner guard), and members read an athlete's Twin within the org via /api/org/[id]/athlete/[uid] — tissue-level injury detail is medical-tier (hidden from performance-only roles), HPI/summary visible to coaches/analysts. Org screen does it all. DB: prisma models + reference/sql-org-graph.sql (member-gated RLS). Next: pending-invite flow for non-users, team-subtree data scoping, board reporting." },
