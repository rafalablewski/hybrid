...
export const AURORA_ICON_PATHS: Record<AuroraIconName, string[]> = {
  ...
  // The mark ALONE. `add` above is the circled variant, and a ring means "it
  // opens something" (the exit-affordance rule) — so a control that GROWS in
  // place could not use it and reached for the fullwidth ＋ text character
  // instead, whose weight and centring belong to whichever mono face happened
  // to be loaded. Drawn here, it takes auroraIconStroke like every other glyph.
  plus: ["M36 20V52M20 36H52"],
