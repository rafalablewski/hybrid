      ...
      {hasEnduranceHistory(sessions) && (
        <>
          ...
          {/* ───── (a) THE LEAD — the section's opener, and it is a SENTENCE:
              how many sports, and which carried them. That is the one thing
              about this section no other block on Today can state, which is
              exactly why the two earlier cuts failed — a per-discipline
              breakdown is a table of contents for the lanes directly below it,
              and a row of totals is the verdict card's own columns one screen
              up (distance especially: only these groups ever carry any). Under
              the sentence, one mono line — this section's own time against its
              own baseline, the only comparison nothing else makes. It carries
              no filter: it reads the screen's period, which the verdict card's
              control writes. ───── */}
