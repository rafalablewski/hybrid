...
export default function AuroraCompetition() {
  ...
  const fieldStyle = { fontFamily: F.mono, fontSize: 15, color: C.chalk, backgroundColor: C.ink, borderWidth: 1, borderColor: C.line, borderRadius: RADIUS.field, padding: 14 } as const;

  return (
    <AuroraScreen>
      <AHeading style={{ fontSize: 26 }}>Competition</AHeading>
      <Text style={{ fontFamily: F.reg, fontSize: 14, color: C.ash, marginTop: 8, lineHeight: 20 }}>Set an event and the optimizer back-solves your season so form peaks on it — finals, not heats.</Text>

      <ACard style={{ marginTop: 16 }}>
        <Text style={{ fontFamily: F.mono, fontSize: 11, textTransform: "uppercase", letterSpacing: 1.2, color: txt(C, C.lime) }}>Add an event</Text>
        ...
        <View style={{ flexDirection: "row", gap: 10, marginTop: 12, alignItems: "flex-end" }}>
          <View style={{ flex: 1 }}>
            <Text style={{ fontFamily: F.mono, fontSize: 10, color: C.ash, marginBottom: 4 }}>Date (YYYY-MM-DD)</Text>
          ...
          <View style={{ flexDirection: "row", gap: 8 }}>
            {events.map((e) => (
              <Pressable key={e.id} onPress={() => setSelected(e.id)} style={{ paddingHorizontal: 14, paddingVertical: 10, borderRadius: RADIUS.field, borderWidth: 1, borderColor: selected === e.id ? C.violet : C.line, backgroundColor: selected === e.id ? `${C.violet}1f` : C.ink2 }}>
                ...
                <Text style={{ fontFamily: F.mono, fontSize: 10, color: C.ash }}>{fmt(e.date)}</Text>
              </Pressable>
            ))}
          </View>
        </ScrollView>
        ...
        <>
          <ACard style={{ marginTop: 14 }}>
            <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
              <Text style={{ fontFamily: F.mono, fontSize: 11, textTransform: "uppercase", letterSpacing: 1.2, color: txt(C, C.amber) }}>{event.name} · {event.sport}</Text>
              <View style={{ backgroundColor: `${plan.landsPeak ? C.lime : C.amber}1f`, borderRadius: RADIUS.pill, paddingHorizontal: 11, paddingVertical: 4 }}>
                <Text style={{ fontFamily: F.mono, fontSize: 11, color: txt(C, plan.landsPeak ? C.lime : C.amber) }}>{plan.landsPeak ? "peak lands ✓" : `peak wk ${plan.peakWeek}`}</Text>
              </View>
            </View>
            <Text style={{ fontFamily: F.mono, fontSize: 13, color: C.chalk, marginTop: 8 }}>{plan.weeksToEvent} weeks out · form at event {Math.round(plan.formAtEvent)}</Text>
          ...
          <ACard style={{ marginTop: 14 }}>
            <Text style={{ fontFamily: F.mono, fontSize: 11, textTransform: "uppercase", letterSpacing: 1.2, color: C.ash }}>Form projection</Text>
            <Text style={{ fontFamily: F.mono, fontSize: 11, color: C.ash, marginTop: 2 }}>fitness − fatigue → form, peaking toward the event.</Text>
            ...
            <Text style={{ fontFamily: F.mono, fontSize: 10, color: C.ash, marginTop: 6 }}>wk 1 → event (peak week in lime)</Text>
          </ACard>
        </>
      )}
