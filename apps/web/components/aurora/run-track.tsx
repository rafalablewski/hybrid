    ...
    <div style={{ maxWidth: "100%", margin: "0 auto", fontFamily: "var(--font-display)", color: C("chalk") }}>
      ...
      <div style={{ ...card, marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", gap: space.ms, marginTop: 16, flexWrap: "wrap" }}>
          <button className="pressable" onClick={toggle} style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: fs.note, background: running ? C("amber") : C("lime"), color: running ? "#12170f" : "var(--on-accent)", border: "none", borderRadius: 999, padding: "12px 24px", cursor: "pointer" }}>
            {running ? t("w.train.runTrack.pause") : elapsed > 0 ? t("w.train.runTrack.resume") : t("w.train.runTrack.startRun")}
          </button>
          <button className="pressable" onClick={reset} disabled={elapsed === 0} style={{ fontFamily: "var(--font-mono)", fontSize: fs.body, color: C("ash"), background: "transparent", border: `1px solid ${C("line")}`, borderRadius: 999, padding: "12px 20px", cursor: elapsed === 0 ? "default" : "pointer", opacity: elapsed === 0 ? 0.5 : 1 }}>
      ...
      <button className="pressable" onClick={save} disabled={saving} style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: fs.note, background: C("lime"), color: "var(--on-accent)", border: "none", borderRadius: 999, padding: "16px 28px", cursor: saving ? "default" : "pointer", opacity: saving ? 0.6 : 1 }}>
