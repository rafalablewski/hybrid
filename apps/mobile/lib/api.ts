...
// ---- analytics reads (coach roster + operator stats) ----
// The mobile Analytics screen (components/aurora/analytics.tsx) renders the same
// three scopes the web dashboard does, off the SAME endpoints — so the two
// clients can't report different numbers. Both degrade to an empty/null value
// rather than throwing: Analytics shows an honest empty state, and a non-coach /
// non-admin simply doesn't render those sections.

/** One athlete on a coach's roster — mirrors the web `RosterRow`. */
...
/** The coach's consented athletes. Empty on any failure (incl. not-a-coach). */
export async function fetchRoster(): Promise<RosterRow[]> {
  try {
    const d = await fetchJson<{ roster?: RosterRow[] }>("/api/coach/roster");
    return d.roster ?? [];
  } catch {
    return [];
  }
