...
const card = { background: C("ink2"), border: `1px solid ${C("line")}`, borderRadius: 28, padding: 18 } as const;
const chip = (color: string, label: string) => <span style={{ background: `color-mix(in srgb, ${color} 14%, transparent)`, color, borderRadius: 999, padding: "3px 12px", fontFamily: "var(--font-mono)", fontSize: 11, marginRight: 6, marginBottom: 4, display: "inline-block" }}>{label}</span>;
const backLink = (onClick: () => void, label: string) => <button onClick={onClick} style={{ background: "none", border: "none", cursor: "pointer", padding: 0, marginBottom: 6, fontFamily: "var(--font-mono)", fontSize: 12, textTransform: "uppercase", letterSpacing: ".06em", color: C("ash") }}>← {label}</button>;
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 16 }}>
            {group.goals.map((g) => (
              <div key={g.id} style={{ ...card, cursor: "pointer" }} onClick={() => setGoalId(g.id)}>
                ...
                <p style={{ fontFamily: "var(--font-mono)", fontSize: 12, lineHeight: 1.5, marginTop: 8, color: C("ash") }}>{g.blurb}</p>
                <div style={{ fontFamily: "var(--font-mono)", fontSize: 11, marginTop: 10, color: g.color }}>{g.plans.length} plans →</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 16 }}>
        {goal.plans.map((p) => (
          <div key={p.id} style={{ ...card, cursor: "pointer" }} onClick={() => pick(p.id)}>
            ...
            <div style={{ fontFamily: "var(--font-mono)", fontSize: 12, margin: "6px 0 10px", color: C("ash") }}>{p.weeks} wks · {p.sessions}×/wk · {p.tag}</div>
            <p style={{ fontSize: 13, lineHeight: 1.5 }}>{p.desc}</p>
  ...
  return (
    <div style={{ maxWidth: "100%", margin: "0 auto", fontFamily: "var(--font-display)", color: C("chalk") }}>
      ...
      <div style={{ ...card, marginBottom: 16 }}>
        <div style={{ fontFamily: "var(--font-mono)", fontSize: 11, textTransform: "uppercase", letterSpacing: ".12em", color: C("lime") }}>Weekly split</div>
        <div style={{ display: "flex", gap: 6, marginTop: 10, flexWrap: "wrap" }}>
          {d.split.map((day, i) => <div key={i} style={{ fontFamily: "var(--font-mono)", fontSize: 12, color: day.toLowerCase() === "rest" ? C("ash") : C("chalk"), background: C("ink"), border: `1px solid ${C("line")}`, borderRadius: 14, padding: "8px 12px" }}>{day}</div>)}
        </div>
      </div>
        ...
        <div key={di} style={{ ...card, marginBottom: 16 }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", marginTop: 10 }}>
            ...
            <tbody>{session.items?.map((it, i) => (
              <tr key={i}>
                <td style={{ fontWeight: 600, fontSize: 14, padding: "10px 0", borderBottom: `1px solid ${C("line")}` }}>{it.name}</td>
                <td style={{ fontFamily: "var(--font-mono)", fontSize: 13, color: C("chalk"), padding: "10px 0", borderBottom: `1px solid ${C("line")}` }}>{it.sr}</td>
                <td style={{ fontFamily: "var(--font-mono)", fontSize: 13, color: C("ash"), padding: "10px 0", borderBottom: `1px solid ${C("line")}` }}>{it.rest}</td>
                <td style={{ fontFamily: "var(--font-mono)", fontSize: 13, color: C("ash"), padding: "10px 0", borderBottom: `1px solid ${C("line")}` }}>{it.rpe}</td>
              </tr>
            ))}</tbody>
          </table>
        </div>
      ...
      <button onClick={enroll} disabled={state === "busy" || state === "done"} style={{ fontWeight: 800, fontSize: 15, background: state === "done" ? C("ink2") : C("lime"), color: state === "done" ? C("lime") : C("ink"), border: state === "done" ? `1px solid ${C("lime")}` : "none", borderRadius: 999, padding: "14px 28px", cursor: state === "busy" || state === "done" ? "default" : "pointer", marginTop: 18 }}>
        {state === "busy" ? "Enrolling…" : state === "done" ? "✓ Enrolled — see Periodize" : `Enroll in ${plan.name} →`}
      </button>
      {state === "error" && <div style={{ fontFamily: "var(--font-mono)", fontSize: 12, marginTop: 10, color: C("amber") }}>Couldn&apos;t enroll — sign in (real auth) and try again.</div>}
    </div>
  );
}

function Info({ label, value }: { label: string; value: string }) {
  return <div style={card}><div style={{ fontFamily: "var(--font-mono)", fontSize: 11, textTransform: "uppercase", letterSpacing: ".12em", color: C("ash") }}>{label}</div><p style={{ fontFamily: "var(--font-mono)", fontSize: 13, lineHeight: 1.5, marginTop: 6, color: C("chalk") }}>{value}</p></div>;
}
