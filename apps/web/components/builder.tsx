...
import { fs, space, INK2, LINE, LIME, CHALK, ASH, VIOLET, RED, ON_ACCENT, disp, cond, mono, Mono, Card } from "@/lib/ui";
import WorkoutBlocks, { uid, type EditableBlock } from "@/components/workout-blocks";
import { useIsMobile } from "@/lib/use-media-query";

const input = { ...mono, fontSize: fs.bodyLg, background: INK2, color: CHALK, border: `1px solid ${LINE}`, borderRadius: 8, padding: "8px 10px", outline: "none", minWidth: 0, boxSizing: "border-box" } as const;
    ...
    <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "2fr 1fr", gap: space.lg, alignItems: "start" }}>
      <div>
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Workout name"
          style={{ ...input, ...disp, fontSize: fs.heading, fontWeight: 800, width: "100%", marginBottom: 8 }} />
        ...
        {msg && <Mono s={{ fontSize: fs.caption, display: "block", marginBottom: 10 }} c={msg.ok ? LIME : RED}>{msg.text}</Mono>}
        <button onClick={save} disabled={saving || blocks.length === 0}
          style={{ ...disp, fontWeight: 800, fontSize: fs.note, background: LIME, color: ON_ACCENT, border: "none", borderRadius: 12, padding: "13px 26px", cursor: saving || !blocks.length ? "default" : "pointer", opacity: saving || !blocks.length ? 0.5 : 1 }}>
      ...
      {/* template library */}
      <Card>
        <Mono s={{ fontSize: fs.micro, textTransform: "uppercase", letterSpacing: ".1em" }} c={VIOLET}>Template library</Mono>
        {templates.length === 0 ? (
          <Mono s={{ fontSize: fs.body, display: "block", marginTop: 10 }}>No templates yet. Build one and save it.</Mono>
        ) : (
          templates.map((t) => (
            <div key={t.id} style={{ marginTop: 10, paddingTop: 10, borderTop: `1px solid ${LINE}` }}>
              <div style={{ ...disp, fontWeight: 700, fontSize: fs.note }}>{t.name}</div>
              <Mono s={{ fontSize: fs.micro }}>{t.blocks.length} blocks{t.description ? ` · ${t.description}` : ""}</Mono>
...
function smallBtn(c: string) {
  return { ...cond, fontSize: fs.caption, fontWeight: 700, textTransform: "uppercase" as const, letterSpacing: ".04em", color: c, background: `${c}1f`, border: `1px solid ${c}55`, borderRadius: 8, padding: "6px 12px", cursor: "pointer" };
}
