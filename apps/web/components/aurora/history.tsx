...
import { sessionVolume, prsForSession, blockSummary, fmtTonnage, type LoggedSession } from "@hybrid/core";
...
export default function AuroraHistory({ sessions, onOpenExercise, onChanged }: { sessions: LoggedSession[]; onOpenExercise?: (name: string) => void; onChanged?: () => void }) {
  ...
  const archivedToggle = (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
      <h1 style={{ fontWeight: 900, fontSize: 26, margin: 0 }}>History</h1>
      <button onClick={toggleArchived} style={{ fontFamily: "var(--font-mono)", fontSize: 12, color: showArchived ? C("amber") : C("ash"), background: "none", border: `1px solid ${showArchived ? C("amber") : C("line")}`, borderRadius: 999, padding: "6px 14px", cursor: "pointer" }}>{showArchived ? "← Back to history" : "Archived ▸"}</button>
    </div>
  );
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12, maxWidth: "100%", margin: "0 auto", fontFamily: "var(--font-display)", color: C("chalk") }}>
      ...
      {list.length === 0 ? (
        <div style={{ ...card, textAlign: "center", padding: 50 }}>
          <div style={{ fontWeight: 800, fontSize: 20 }}>{showArchived ? "No archived workouts" : "No sessions yet"}</div>
          <p style={{ fontFamily: "var(--font-mono)", fontSize: 14, marginTop: 10, color: C("ash") }}>{showArchived ? "Workouts you archive show up here." : "Log your first workout — it'll appear here and feed your dashboard."}</p>
        </div>
      ) : list.map((s) => {
          ...
          <div key={s.id} style={card}>
            <div onClick={() => !showArchived && setOpenId(s.id)} style={{ cursor: showArchived ? "default" : "pointer" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
                ...
                <span style={{ fontFamily: "var(--font-mono)", fontSize: 12, color: C("ash") }}>{fmtDate(s.startedAt)}</span>
              </div>
              ...
              {s.blocks.map((b, i) => (
                <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderTop: `1px solid ${C("line")}`, fontFamily: "var(--font-mono)", fontSize: 13 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 10, paddingTop: 10, borderTop: `1px solid ${C("line")}` }}>
              {!showArchived ? <button onClick={() => setOpenId(s.id)} style={{ fontFamily: "var(--font-mono)", fontSize: 12, color: C("ash"), background: "none", border: "none", cursor: "pointer" }}>Open the full breakdown →</button> : <span />}
