...
export default function AuroraRunning() {
  ...
  const header = (
    <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
      ...
      <AHeading style={{ fontSize: 26 }}>{t("nav.running")}</AHeading>
    ...
    return (
      <AuroraScreen refreshing={refreshing} onRefresh={load}>
        ...
        <ACard style={{ marginTop: 16, alignItems: "center", paddingVertical: 32 }}>
          ...
          <Text style={{ fontFamily: F.reg, fontSize: 14, color: C.ash, marginTop: 8, textAlign: "center", lineHeight: 19 }}>{t("running.emptyBody")}</Text>
        </ACard>
      </AuroraScreen>
    );
  ...
  return (
    <AuroraScreen refreshing={refreshing} onRefresh={load}>
      ...
      <ACard style={{ marginTop: 14 }}>
        <Text style={{ fontFamily: F.mono, fontSize: 11, textTransform: "uppercase", letterSpacing: 1.2, color: txt(C, C.blue) }}>{t("running.weeklyMileage")}</Text>
      ...
      {hasEffort && (
        <ACard style={{ marginTop: 14 }}>
          <Text style={{ fontFamily: F.mono, fontSize: 11, textTransform: "uppercase", letterSpacing: 1.2, color: txt(C, C.lime) }}>{t("running.effortSplit")}</Text>
      ...
      {paceMoves.length > 0 && (
        <ACard style={{ marginTop: 14 }}>
          <Text style={{ fontFamily: F.mono, fontSize: 11, textTransform: "uppercase", letterSpacing: 1.2, color: txt(C, C.blue) }}>{t("session.paceTrend")} · {active}</Text>
          {paceMoves.length > 1 && (
            <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6, marginTop: 10 }}>
              {paceMoves.map((m) => { const on = active === m; return (
                <Pressable key={m} onPress={() => setMove(m)} style={{ paddingHorizontal: 12, paddingVertical: 7, borderRadius: RADIUS.pill, borderWidth: 1, borderColor: on ? C.blue : C.line, backgroundColor: on ? `${C.blue}1a` : "transparent" }}>
                  <Text style={{ fontFamily: F.semi, fontSize: 12, color: on ? txt(C, C.blue) : C.ash }}>{m}</Text>
                </Pressable>
              ); })}
            </View>
          )}
      ...
      <ACard style={{ marginTop: 14 }}>
        <Text style={{ fontFamily: F.mono, fontSize: 11, textTransform: "uppercase", letterSpacing: 1.2, color: txt(C, C.blue) }}>{t("running.byMove")}</Text>
        ...
        {stats.map((r) => (
          <View key={r.move} style={{ flexDirection: "row", paddingVertical: 8, borderTopWidth: 1, borderTopColor: C.line }}>
            <Text style={{ flex: 2, fontFamily: F.mono, fontSize: 13, color: C.chalk }}>{r.move}</Text>
            <Text style={{ flex: 1, textAlign: "center", fontFamily: F.mono, fontSize: 13, color: C.ash }}>{r.distanceKm}</Text>
            <Text style={{ flex: 1, textAlign: "center", fontFamily: F.mono, fontSize: 13, color: C.ash }}>{r.longestKm || "–"}</Text>
            <Text style={{ flex: 1, textAlign: "center", fontFamily: F.mono, fontSize: 13, color: txt(C, r.bestPaceSecPerKm != null ? C.blue : C.ash) }}>{r.bestPaceSecPerKm != null ? paceClock(r.bestPaceSecPerKm) : "–"}</Text>
          </View>
        ))}
      </ACard>
    </AuroraScreen>
...
function PaceBars({ series }: { series: number[] }) {
  const { palette: C } = useTheme();
  if (series.length < 2) return <Text style={{ fontFamily: F.mono, fontSize: 13, color: C.ash, marginTop: 12 }}>Log this run a few times to see a pace trend.</Text>;
  const max = Math.max(...series), min = Math.min(...series), range = max - min || 1;
  const delta = series[series.length - 1]! - series[0]!;
  return (
    <View style={{ marginTop: 12 }}>
      <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 8 }}>
        <Text style={{ fontFamily: F.mono, fontSize: 13, color: C.ash }}>{`${paceClock(series[series.length - 1]!)} /km`}</Text>
        <Text style={{ fontFamily: F.mono, fontSize: 13, color: txt(C, delta <= 0 ? C.lime : C.amber) }}>{`${delta <= 0 ? "−" : "+"}${paceClock(Math.abs(delta))} · ${series.length}×`}</Text>
      </View>
...
function Legend({ c, label }: { c: string; label: string }) {
  ...
  return (
    <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
      ...
      <Text style={{ fontFamily: F.mono, fontSize: 12, color: C.ash }}>{label}</Text>
    </View>
  );
}
