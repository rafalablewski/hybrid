...
export default function AuroraTrends({ sessions, onOpenExercise, onOpenVolume }: { sessions: LoggedSession[]; onOpenExercise?: (name: string) => void; onOpenVolume?: () => void }) {
  ...
  const frameHead = (color: string, kicker: string) => <div style={{ fontFamily: "var(--font-mono)", fontSize: 11, textTransform: "uppercase", letterSpacing: ".12em", color: C(color), marginBottom: 10 }}>{kicker}</div>;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16, maxWidth: 1000, margin: "0 auto", fontFamily: "var(--font-display)", color: C("chalk") }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: 16 }}>
        <div style={card}>{frameHead("lime", "Weekly working sets · last 8 wk")}
          <ResponsiveContainer width="100%" height={200}><BarChart data={weekData}><CartesianGrid stroke={LINE} strokeDasharray="3 3" /><XAxis dataKey="w" stroke={ASH} style={{ ...mono, fontSize: 11 }} /><YAxis stroke={ASH} style={{ ...mono, fontSize: 11 }} width={32} /><Tooltip contentStyle={tip} formatter={(v) => `${v} sets`} /><Bar dataKey="sets" fill={LIME} radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer>
        </div>
        <div style={card}>{frameHead("blue", `Weekly tonnage · ${units === "kg" ? "tonnes" : "k lb"}`)}
          <ResponsiveContainer width="100%" height={200}><BarChart data={weekData}><CartesianGrid stroke={LINE} strokeDasharray="3 3" /><XAxis dataKey="w" stroke={ASH} style={{ ...mono, fontSize: 11 }} /><YAxis stroke={ASH} style={{ ...mono, fontSize: 11 }} width={32} /><Tooltip contentStyle={tip} formatter={(v) => `${v} ${units === "kg" ? "t" : "k lb"}`} /><Bar dataKey="t" fill={BLUE} radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer>
        </div>
      </div>

      <div style={card}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
          <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, textTransform: "uppercase", letterSpacing: ".12em", color: C("blue") }}>Muscle breakdown · this week</span>
          {onOpenVolume && <button onClick={onOpenVolume} style={{ fontFamily: "var(--font-mono)", fontSize: 12, color: C("lime"), background: "none", border: "none", cursor: "pointer" }}>Volume detail →</button>}
        </div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 12 }}>
          {muscles.map((m) => { const c = C(muscleZone(m.zone)); const on = m.muscle === focusMuscle; return (
            <button key={m.muscle} onClick={() => setSelMuscle(m.muscle)} style={{ display: "flex", alignItems: "center", gap: 6, border: `1px solid ${on ? c : `color-mix(in srgb, ${c} 40%, transparent)`}`, background: `color-mix(in srgb, ${c} ${on ? 18 : 8}%, transparent)`, borderRadius: 999, padding: "6px 12px", cursor: "pointer" }}>
              <span style={{ fontFamily: "var(--font-mono)", fontSize: 12, color: C("chalk") }}>{MUSCLE_LABEL[m.muscle] ?? m.muscle}</span>
              <span style={{ fontFamily: "var(--font-mono)", fontSize: 12, fontWeight: 800, color: c }}>{m.sets}</span>
            </button>
          ); })}
        </div>
        <div style={{ marginTop: 14 }}>
          <div style={{ fontFamily: "var(--font-mono)", fontSize: 10, textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 4, color: C("ash") }}>{MUSCLE_LABEL[focusMuscle] ?? focusMuscle} · weekly sets · 8 wk</div>
          <ResponsiveContainer width="100%" height={120}><BarChart data={muscleWeeks.map((s, i) => ({ w: fmtWeek(weeks[i]?.weekStart ?? ""), sets: s }))}><CartesianGrid stroke={LINE} strokeDasharray="3 3" /><XAxis dataKey="w" stroke={ASH} style={{ ...mono, fontSize: 10 }} /><YAxis stroke={ASH} style={{ ...mono, fontSize: 10 }} width={26} allowDecimals={false} /><Tooltip contentStyle={tip} formatter={(v) => `${v} sets`} /><Bar dataKey="sets" fill={BLUE} radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer>
        </div>
        {advice.length > 0 && (
          <div style={{ fontFamily: "var(--font-mono)", fontSize: 12, color: C("ash"), marginTop: 10 }}>
      ...
      <div style={card}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
          <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, textTransform: "uppercase", letterSpacing: ".12em", color: C("lime") }}>Exercise analytics</span>
