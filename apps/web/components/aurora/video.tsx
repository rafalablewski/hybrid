...
export default function AuroraVideo() {
  ...
  return (
    <div style={{ maxWidth: "100%", margin: "0 auto", fontFamily: "var(--font-display)", color: C("chalk"), display: "grid", gap: space.lg }}>
      <h1 style={{ fontWeight: 900, fontSize: fs.display, margin: 0 }}>Technique</h1>
      <div style={card}>
        <div style={{ fontFamily: "var(--font-mono)", fontSize: fs.micro, textTransform: "uppercase", letterSpacing: ".12em", color: C("violet") }}>Video intelligence · markerless motion analysis</div>
        <p style={{ fontSize: fs.body, lineHeight: 1.5, marginTop: 6 }}>An on-device pose model turns a phone clip into keypoints; the engine scores technique (depth, rep count, L/R asymmetry) and feeds asymmetry into the Twin&apos;s injury risk. Lab-grade biomechanics, phone-first.</p>
        <div style={{ fontFamily: "var(--font-mono)", fontSize: fs.micro, marginTop: 12, color: C("ash") }}>Capture runs on the phone (native pose model). Analyses you record appear below.</div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 14 }}>
        {analyses.length === 0 && <span style={{ fontFamily: "var(--font-mono)", fontSize: fs.body, color: C("ash") }}>No analyses yet — record a clip on the phone to see the engine output.</span>}
        {analyses.map((a) => (
          <div key={a.id} style={card}>
            ...
            <div style={{ fontFamily: "var(--font-mono)", fontSize: fs.micro, marginTop: 2, color: C("ash") }}>technique score</div>
