...
describe("the screen-head guard — no head opts out of its rung", () => {
  ...
  it("gives no bottom-nav tab root a hero", () => {
    ...
    // hero at its root, and Train was moved onto one on the argument that
    // Performance and Feed have one — true, but they wear it as PUSHED screens.
    // A rule that lives only in prose is a rule the next reader finds by
    // reading the exception.
    //
    // The five bottom-nav destinations, by the shape each one legitimately has:
    //  - Today, Profile, Train and Messages are tab roots END TO END — no hero
    //    at all, so the plain assertion is the whole truth for them.
