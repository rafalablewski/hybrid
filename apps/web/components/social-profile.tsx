        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
          {(["friends", "following", "followers", "requests"] as const).map((tb) => (
            <Pill key={tb} active={tab === tb} onClick={() => setTab(tb)}>
              ...
              {conns && (tb === "requests" ? conns.requests?.length : tb === "friends" ? conns.friends?.length : tb === "following" ? conns.following?.length : conns.followers?.length) ? ` ${(tb === "requests" ? conns.requests : tb === "friends" ? conns.friends : tb === "following" ? conns.following : conns.followers).length}` : ""}
            </Pill>
          ))}
        </div>
        {connList.length === 0 ? (
          <EmptyState title={tab === "requests" ? "No pending requests" : `No ${tab} yet`} sub={tab === "friends" ? "Follow someone who follows you back to become friends." : "Find people on the Find friends tab."} />
        ) : (
          connList.map((u: any) => (
            <div key={u.id || u.followerId} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 4px", borderBottom: `1px solid ${C("line")}` }}>
              <button onClick={() => u.handle && setDrawer(u.handle)} style={{ background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 12, flex: 1, minWidth: 0, textAlign: "left", padding: 0 }}>
                <Avatar url={u.avatarUrl} name={u.displayName} handle={u.handle} size={40} />
                <div style={{ minWidth: 0 }}>
                  <div style={{ color: C("chalk"), fontFamily: "var(--font-display)", fontWeight: 600 }}>{u.displayName || `@${u.handle}`}</div>
                  <div style={{ color: C("ash"), fontSize: 12, fontFamily: "var(--font-mono)" }}>@{u.handle}{u.friend ? " · friend" : u.closeFriend ? " · close" : ""}</div>
                </div>
              </button>
              {tab === "requests" ? (
                <div style={{ display: "flex", gap: 6 }}>
                  <Btn small onClick={() => busy.run(u.followerId, async () => { await jsend("/api/social/follow/respond", "POST", { followerId: u.followerId, action: "approve" }); await load(); })}>Accept</Btn>
                  <Btn ghost small onClick={() => busy.run(u.followerId, async () => { await jsend("/api/social/follow/respond", "POST", { followerId: u.followerId, action: "deny" }); await load(); })}>Deny</Btn>
                </div>
              ) : tab === "following" ? (
                <Btn ghost small tone={u.closeFriend ? "amber" : "lime"} onClick={() => busy.run(u.id, async () => { await jsend("/api/social/close-friend", "POST", { followeeId: u.id, close: !u.closeFriend }); await load(); })}>
                  {u.closeFriend ? "★ Close" : "☆ Close"}
                </Btn>
              ) : tab === "followers" && !u.friend ? (
                <Btn small onClick={() => busy.run(u.id, async () => { await jsend("/api/social/follow", "POST", { followeeId: u.id }); await load(); })}>Follow back</Btn>
              ) : null}
            </div>
          ))
        )}
