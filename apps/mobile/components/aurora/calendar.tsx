...
import { fs, space, tracking, F, FIXED_FONT_SCALE, PressScale as Pressable, ty } from "../../lib/ui";
  ...
  return (
    <AuroraScreen refreshing={refreshing} onRefresh={load} hero={{ rank: "title", title: t("nav.calendar") }}>
      ...
      {selEvents.map((e) => (
        <ACard key={e.id} style={{ marginBottom: 12 }}>
          ...
          <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: space.sm, marginTop: space.xs }}>
            <Text style={{ flex: 1, fontFamily: F.bold, fontSize: fs.bodyLg, color: C.chalk }}>{eventName(e)}</Text>
      ...
      <Pressable
        ...
        hitSlop={8}
        style={{ flexDirection: "row", alignItems: "center", gap: space.sm, minHeight: 44, alignSelf: "flex-start", marginTop: space.xxs }}
      >
        <Text style={{ fontFamily: F.black, fontSize: fs.bodyLg, color: C.ash }}>＋</Text>
        <Text style={ty(C, "kicker")}>{t("w.event.add")}</Text>
      </Pressable>
      </DayDetail>
