...
import { CHECKIN_STEP_COUNT, CHECKIN_METRICS } from "./checkin-flow";
...
describe("checkin flow", () => {
  ...
  // The re-log cadence moved to readiness-reads.ts (`readGate`) — see
  // readiness-reads.test.ts. It is no longer a flat window.
});
import {
  quickCheckinMetrics,
  answeredMetrics,
...
describe("a write may only touch what it answered", () => {
  it("a readiness tap sends one metric and no nulls", () => {
    ...
    expect(quickCheckinPatch(4)).toEqual({ energy: 4, newRead: true });
    // …and no OTHER metric: the tap answered one question.
    expect(Object.keys(quickCheckinPatch(4)).filter((k) => k !== "newRead")).toEqual(["energy"]);
    expect(quickCheckinPatch(9)).toEqual({ energy: 5, newRead: true });
  });
