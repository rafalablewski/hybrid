...
import { F, LoadSwap, Loading, PressScale as Pressable, ty} from "../../lib/ui";
...
function ListEnd({ text }: { text: string }) {
  const { palette: C } = useTheme();
  return (
    <Text style={{ ...ty(C, "kicker"), textAlign: "center", paddingVertical: 14  }}>
      {text}
    </Text>
  );
}
                ...
                <View style={{ marginTop: 8 }}>
                  {p.preview.map((w: ProgramPreviewWeek, wi: number) => (
                    <View key={wi} style={{ marginBottom: 8 }}>
                      <Text style={ty(C, "kicker")}>{t("w.coaches.week")} {wi + 1}</Text>
