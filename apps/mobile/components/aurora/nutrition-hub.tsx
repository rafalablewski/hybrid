...
function StatTile({ glyph, tint, value, unit, caption, name, onPress, palette: C }: {
  ...
  return (
    ...
    >
      ...
      <Text style={{ fontFamily: F.mono, fontSize: fs.nano, letterSpacing: 1.1, textTransform: "uppercase", color: C.ash, marginTop: 1 }} numberOfLines={1}>{caption}</Text>
...
export function NutritionHubBento({ series, avgKcal, weightKg, ratePerWeek, mealCount, productCount, onOpen }: {
  ...
  const keyLabel = { fontFamily: F.mono, fontSize: fs.nano, letterSpacing: 1.1, textTransform: "uppercase" as const, color: C.ash };
