...
export default function TodayWidgets({ onNavigate }: { onNavigate?: (screen: string) => void }) {
  ...
  const widget = { minWidth: 0, aspectRatio: "1 / 1", borderRadius: 26, border: `1px solid ${C("line")}`, background: C("ink2"), boxShadow: "var(--shadow-card)", padding: 18, display: "flex", flexDirection: "column" as const, cursor: "pointer", textAlign: "left" as const, color: C("chalk"), overflow: "hidden" as const };
