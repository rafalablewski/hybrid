...
export const SECURITY_CONTROLS: SecurityControl[] = [
  ...
  { id: "authn-mfa", category: "Authentication", severity: "high", status: "manual", title: "Multi-factor authentication (TOTP) — enrollment deferred until domain", detail: "TOTP 2FA is implemented end-to-end: enroll/verify/remove in account settings, and a sign-in step-up that prompts for a code when a verified factor exists (a no-op otherwise, so it can't lock anyone out). The decision logic (stepUpRequired, code validation) is unit-tested. DEFERRED BY DECISION: per-user enrollment + the in-browser verification of the challenge flow are intentionally on hold until the production domain is registered — do it then. (Supabase TOTP is enabled by default; no code change is required to switch it on later.)", evidence: "core mfa.ts (tested) + components/account/mfa.tsx + login step-up; enroll once the domain is live" },
