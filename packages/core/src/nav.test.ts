...
describe("navForPersona", () => {
  ...
  it("casual (Average Joe) is exactly the curated lean set — the done deal", () => {
    ...
    // own Profile — and the pre-built Plan library (browse & enroll is free;
    // periodizing it is paid).
    expect(navForPersona("casual").map((i) => i.id).sort()).toEqual(
      ["calendar", "checkin", "history", "log", "notifications", "nutrition", "plans", "profile", "progress", "runtrack", "settings", "statistics", "timer", "today"],
    );
  });
