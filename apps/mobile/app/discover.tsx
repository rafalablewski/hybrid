...
import { userPagePath, type PersonCard } from "@hybrid/core";
...
export default function DiscoverScreen() {
  ...
  return (
    <AuroraScreen hero={{ rank: "title", title: t("w.social.findFriends"), meta: [t("w.social.findFriendsSub")] }}>
      ...
      {results !== null ? (
        <ACard style={cardStack}>{results.length === 0 ? <Empty title={t("w.social.noOneFound")} sub={t("w.social.noOneFoundSub")} /> : results.map((p) => <Row key={p.userId} p={p} onChanged={refresh} onOpen={(h) => { if (h) router.push(userPagePath(h)); }} />)}</ACard>
      ) : (
        <>
          ...
          <ACard style={cardStack}>{sugg.length === 0 ? <Empty title={t("w.social.noSuggestions")} sub={t("w.social.noSuggestionsSub")} /> : sugg.map((p) => <Row key={p.userId} p={p} onChanged={refresh} onOpen={(h) => { if (h) router.push(userPagePath(h)); }} />)}</ACard>
        </>
      )}
    </AuroraScreen>
  );
