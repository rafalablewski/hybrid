...
export const CAPABILITIES: Capability[] = [
  ...
  { id: "signal-ontology", area: "Core", status: "shipped", title: "Signal ontology (Athlete Twin foundation)", detail: "Universal time-series in @hybrid/core: every measurement (HRV/RHR/sleep today; GPS load, force-plate, blood tomorrow) is one Signal shape from any source, with rolling baselines + oriented z-scores. A toBiometrics() adapter feeds the readiness engine unchanged. DB: prisma Signal model + reference/sql-signal-table.sql (run in Supabase). Pure + unit-tested." },
  { id: "hpi", area: "Core", status: "shipped", title: "HPI — Hybrid Performance Index", detail: "The headline 0..100 number that fuses strength + endurance + recovery (distinct from readiness, which is muscle-only). Reports its three pillars, its limiter, and a band; sport-weightable. Computable in core + unit-tested; surfacing in dashboards/API is next." },
  ...
  { id: "athlete-twin", area: "Data", status: "planned", title: "Athlete Digital Twin + Performance State", detail: "Materialize one fused per-athlete model from the Signal ontology: readiness, HPI, fatigue-by-tissue, trajectory, risk — with attribution ('why did it move'). The cockpit the coach opens first. Foundation (signal-ontology, HPI) shipped; ingestion + materialization + UI next." },
  { id: "injury-risk", area: "AI", status: "planned", title: "Tissue-level injury risk + return-to-play", detail: "Per-tissue, per-athlete calibrated risk with explanations, plus gated, auditable RTP protocols. Versioned models with offline eval before ship. The line item a club CFO funds first." },
