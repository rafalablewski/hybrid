...
/**
 * The ROOM MARK (web) — a muscle group drawn as the body it trains, for the
 ...
 *
 * Twin of apps/mobile/components/aurora/body-mark.tsx.
 */
export default function AuroraBodyMark({
  ...
  return (
    <svg
      ...
      fill="none"
      role="img"
      aria-label={top ? MUSCLE_SHORT[top] : undefined}
      style={{ display: "block", flex: "none" }}
    >
      {/* the unlit body: silhouette + head */}
      {figure.outline.map((poly, i) => (
        <polygon key={i} points={poly.map((q) => `${q.x},${q.y}`).join(" ")} fill={silhouette} />
      ))}
