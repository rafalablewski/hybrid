  ...
  if (b.t === "note") {
    return (
      <View style={{ borderLeftWidth: 3, borderLeftColor: palette.amber, backgroundColor: `${palette.amber}12`, borderRadius: 8, padding: 12 }}>
        ...
        <Text style={{ fontFamily: F.reg, fontSize: 13.5, lineHeight: 21, color: palette.chalk, marginTop: 4 }}>{b.text}</Text>
      </View>
    );
  }
  ...
  if (b.t === "term") {
    return (
      <View style={{ paddingLeft: 12, borderLeftWidth: 2, borderLeftColor: palette.line }}>
        <Text style={{ fontFamily: F.bold, fontSize: 14.5, color: txt(palette, palette.lime), marginBottom: 3 }}>{b.term}</Text>
        <Text style={{ fontFamily: F.reg, fontSize: 13.5, lineHeight: 21, color: palette.chalk }}>{b.text}</Text>
      </View>
    );
  }
  ...
  return (
    <View style={{ position: "relative" }}>
      <View style={{ backgroundColor: palette.ink, borderWidth: 1, borderColor: palette.line, borderRadius: 10, padding: 12, paddingRight: 64 }}>
        <Mono color={palette.chalk} style={{ fontSize: 12.5, lineHeight: 20 }}>{lines}</Mono>
      </View>
      <Pressable
        onPress={copy}
        hitSlop={8}
