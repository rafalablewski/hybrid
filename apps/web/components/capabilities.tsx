...
import { LINE, LIME, CHALK, ASH, AMBER, disp, mono, Mono, Card, Chip } from "@/lib/ui";
...
export default function CapabilitiesScreen() {
  ...
  return (
    <div>
        ...
        {order.map((st) => {
          ...
          return (
            <Card key={st} style={{ borderTop: `2px solid ${m.color}` }}>
              <Mono s={{ fontSize: 11, textTransform: "uppercase", letterSpacing: ".1em" }} c={m.color}>
                {m.label}
              </Mono>
              <div style={{ ...disp, fontWeight: 800, fontSize: 32, color: CHALK, margin: "4px 0 2px" }}>{n}</div>
              <Mono s={{ fontSize: 11 }}>{m.blurb}</Mono>
            </Card>
          );
        })}
      </div>
      ...
      {order.map((st) => {
        ...
        return (
          <div key={st} style={{ marginBottom: 8 }}>
            <Mono s={{ fontSize: 11, textTransform: "uppercase", letterSpacing: ".1em", display: "block", margin: "10px 0" }} c={m.color}>
      ...
      <Mono s={{ fontSize: 11, display: "block", marginTop: 18 }} c={ASH}>
  ...
  return (
    <Card style={{ borderLeft: `3px solid ${color}` }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 }}>
        <div style={{ ...disp, fontWeight: 700, fontSize: 15 }}>{cap.title}</div>
      ...
      {cap.blockedBy && (
        <div style={{ marginTop: 10, padding: "8px 10px", borderRadius: 8, background: `${AMBER}12`, border: `1px solid ${AMBER}40` }}>
          <Mono s={{ fontSize: 11, textTransform: "uppercase", letterSpacing: ".08em" }} c={AMBER}>
            Needs
          </Mono>
          <Mono s={{ fontSize: 12, lineHeight: 1.45, display: "block", marginTop: 3 }} c={CHALK}>
            {cap.blockedBy}
          </Mono>
        </div>
      )}
